...
└─────────────┘        └──────────────┘        └─────────────┘
                       ...
                       └──────────────────────────────┘
                                      ...
                                      ▼
                       ┌──────────────────────────────┐
                       │ Hollyhop Web Form (refunds)  │
                       │   /Payment/EditStudyRefund   │
                       └──────────────────────────────┘
```
...
Транзакция/Сделка/Счет?
    ...
    Это возврат?
      ...
      └─ Да → возврат
               ↓
             GetPayments → найти исходный Paid-платеж
               ↓
             AuthByKey → открыть внутреннюю форму Hollyhop
               ↓
             GET /Payment/EditStudyRefund?payerId={clientId}
               ↓
             POST /Payment/EditStudyRefund
```

**Как теперь работает возврат**:
- Публичный `Api/V2/AddPayment` для типа `Возврат` не подходит: Hollyhop отвечает `Unknown payment type` или `Type "Возврат" is not supported`.
- Поэтому возврат создается через внутреннюю web-форму Hollyhop, тем же маршрутом, который использует интерфейс CRM.
- В качестве `payerId` используется `clientId` ученика в Hollyhop.
- Филиал для возврата берется из текущего филиала ученика через `GetStudents`.
- Перед созданием возврата скрипт ищет последний подходящий `Paid`-платеж через `GetPayments`, чтобы унаследовать корректный `PaymentMethodId` и сформировать понятный комментарий.
- Для web-сценария создается временная cookie-сессия в папке `logs/`, после чего выполняются `AuthByKey`, чтение формы и отправка POST в Hollyhop.
- Сумма возврата отправляется в положительном виде, а Hollyhop сам создает платеж типа `Возврат` с отрицательным значением.
...
**Статусы возврата**:
- Если в статусе счета есть слово `возврат`, `add_payment.php` идет не в обычный `AddPayment`, а в специальную ветку создания возврата.
- Для сценариев вида `Полный возврат` сумма берется из `BILL_PRICE`, а дата возврата нормализуется перед отправкой в web-форму Hollyhop.
